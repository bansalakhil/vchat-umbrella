...
var Chat = {
  ...
  getMboxContainer: function getMboxContainer(name) {
    return Chat.getAllMboxContainer.filter("[data-mbox=" + name + "]");
  },

  pushMsgToMboxContainer: function pushMsgToMboxContainer(username, container, msg, payload, options) {
    // console.log(msg)
  ...
  displayNotification: function displayNotification(username, payload) {
  ...
  getUserFullName: function getUserFullName(username) {
  ...
  getNotificationBoxFor: function getNotificationBoxFor(username) {
  ...
  resetNotificationFor: function resetNotificationFor(username) {
  ...
  displayUserExited: function displayUserExited(username) {
  ...
  markSeen: function markSeen(messages) {
    // alert(messages)
  ...
  addLinkInfo: function addLinkInfo(payload) {
  ...
  textToLinks: function textToLinks(text) {
...
var _webStaticJsChat = require("web/static/js/chat");
...
var channel = socket.channel("chat:*", {});
var chatLobby = _webStaticJsChat.Chat.getLobbyMbox;
_webStaticJsChat.Chat.channel = channel;
...
channel.on("user:entered_in_lobby", function (payload) {
  // get the name of the user who joined the lobby
  var name = _webStaticJsChat.Chat.getUserFullName(payload.user);
  var $joinedMsg = $("<div>");
  var currentTime = new Date().toLocaleString();
  var msg = name + " joined ";
  var $timestampContainer = $("<span>", {
  ...
  _webStaticJsChat.Chat.setUserActive(payload.user);
  _webStaticJsChat.Chat.setInactiveUserStatus(payload.inactive_users);
});

channel.on("chat:old_messages", function (payload) {

  $.each(payload.received_messages, function () {
    _webStaticJsChat.Chat.displayMessage(this, {
      highlight: false
    });
  });
});

channel.on("chat:new_msg", function (payload) {
  _webStaticJsChat.Chat.displayMessage(payload, {
    highlight: true
  });
});

channel.on("chat:user_status", function (payload) {
  _webStaticJsChat.Chat.setInactiveUserStatus(payload.inactive_users);
...
channel.on("chat:user_offline", function (payload) {
  _webStaticJsChat.Chat.setUserInactive(payload.username);
  channel.push("chat:record_last_activity", {
    msg: "..."
  });
});

channel.on("chat:link_info", function (payload) {
  _webStaticJsChat.Chat.addLinkInfo(payload);
});

var $msgBox = _webStaticJsChat.Chat.getMsgBox;
...
$(function () {
  _webStaticJsChat.Chat.getUsers.on('click', function () {
    // console.log(this)
    //
    var $this = $(this);
    var group_name = $this.attr("data-group-name");
    var messages = _webStaticJsChat.Chat.getMboxContainer(group_name).find("[data-behaviour=msg]").filter("[data-seen=false]");
    _webStaticJsChat.Chat.markSeen(messages);
