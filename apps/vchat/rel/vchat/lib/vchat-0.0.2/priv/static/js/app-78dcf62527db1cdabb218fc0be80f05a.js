...
channel.join().receive("ok", function (resp) {
  console.log("Joined successfully", resp);channel.push("chat:old_messages", { msg: "..." });
...
channel.on("chat:old_messages", function (payload) {
  ...
  $.each(payload.received_messages, function () {
    _webStaticJsChat.Chat.displayMessage(this, { highlight: false });
  });
});

channel.on("chat:new_msg", function (payload) {
  _webStaticJsChat.Chat.displayMessage(payload, { highlight: true });
});
